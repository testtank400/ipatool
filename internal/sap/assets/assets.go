...
func download(ctx context.Context) (Bundle, error) {
	files, err := downloadFiles(ctx, requiredFiles, "download Apple SAP assets")
	if err != nil {
		return Bundle{}, err
	}
...
func downloadFiles(ctx context.Context, specs []fileSpec, errPrefix string) (map[string][]byte, error) {
